...
const Endorsements: React.FC<EndorsementsProps> = ({ endorsements }) => {
	...
	const getLocalizedText = (key: string) => {
		...
		return (
			translations[key as keyof typeof translations]?.[language] ||
			translations[key as keyof typeof translations]?.lv
		)
	}
			...
			entries => {
				entries.forEach(entry => {
					if (entry.isIntersecting) {
						const cards =
							entry.target.querySelectorAll('.endorsement-card')
	...
	const generateLogo = (company: string) => {
		...
		return (
			<div
				className={`w-16 h-16 bg-gradient-to-br ${colors[colorIndex]} rounded-xl flex items-center justify-center shadow-lg`}
			>
						...
						>
							...
							<CardContent className='p-8 relative z-10'>
								...
								<Button
									...
									onClick={() => {
										window.open(endorsement.link, '_blank')
									}}
								>
